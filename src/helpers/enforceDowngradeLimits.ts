  ...
  try {
    ...
    if (maxBranches !== undefined) {
      ...
      if (maxBranches !== -1 && activeBranches.length > maxBranches) {
        ...
        console.log(
          `[SaaS] Se suspendieron ${branchesToSuspend.length} sucursales por downgrade en empresa ${companyId}`,
        );
        ...
        if (branchesToReactivate.length > 0) {
          console.log(
            `[SaaS] Se reactivaron ${branchesToReactivate.length} sucursales por UPGRADE en empresa ${companyId}`,
          );
        }
      }
    }
    ...
    if (maxUsers !== undefined) {
      ...
      if (maxUsers !== -1 && activeUsers.length > maxUsers) {
        ...
        const inactiveUsers = await User.find({
          companyId: companyId,
          active: false,
          deactivationReason: 'Límite de plan excedido (Downgrade)',
          role: { $ne: 'companyAdmin' },
        }).sort({ createdAt: 1 });
