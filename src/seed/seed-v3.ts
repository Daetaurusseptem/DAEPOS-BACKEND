...
async function seedV3() {
  try {
    ...
    const sysadmin = await new User({
      username: 'sysadmin',
      email: 'sysadmin@daepoint.com',
      password: hashedPassword,
      name: 'Super Admin DaePoint',
      role: 'sysadmin',
    }).save();
    ...
    const planBasic = await new SubscriptionPlan({
      ...
      features: ['Ventas Retail', 'Inventario Básico'],
    }).save();
    ...
    const planPro = await new SubscriptionPlan({
      ...
      features: ['Ventas Hospitalidad', 'KDS', 'Multisucursal'],
    }).save();

    const planEnterprise = await new SubscriptionPlan({
      name: 'Enterprise',
      stripeProductId: 'prod_enterprise_123',
      maxBranches: 10,
      maxUsers: 50,
      maxActiveRegisters: 20,
      features: ['Todo Incluido', 'Soporte 24/7'],
    }).save();

    const planManual = await new SubscriptionPlan({
      name: 'Plan Corporativo B2B',
      stripeProductId: 'prod_manual_123',
      isCustom: true,
      maxBranches: 99,
      maxUsers: 99,
      maxActiveRegisters: 99,
    ...
    const companyA = await new Company({
      name: 'Grupo Alsea B2B S.A.',
      description: 'Consorcio Multigiro (Retail + Hospitality)',
      address: 'Torre Mayor Piso 42',
      tel: '555-1000',
      email: 'ceo@alsea.com',
      adminId: sysadmin._id,
      maxActiveRegisters: 20,
      maxCashLimit: 50000,
      planId: planManual._id,
      planType: 'Plan Corporativo B2B',
      isActive: true,
      subscriptionStatus: 'manual',
      manualOverride: true,
      currentPeriodEnd: moment().add(1, 'month').toDate(),
    ...
    const companyC = await new Company({
      ...
      email: 'hola@floral.com',
      planId: planBasic._id,
      planType: 'Básico',
      isActive: true,
      subscriptionStatus: 'trialing',
      currentPeriodEnd: moment().add(3, 'days').toDate(),
      adminId: sysadmin._id,
    }).save();
    ...
    const companyD = await new Company({
      name: 'Kiosko Fallido',
      email: 'adios@kiosko.com',
      planId: planBasic._id,
      planType: 'Básico',
      isActive: false,
      subscriptionStatus: 'past_due',
      currentPeriodEnd: moment().subtract(10, 'days').toDate(),
      adminId: sysadmin._id,
    }).save();
    ...
    const branchRetail = await new Branch({
      ...
      tel: '555-2001',
      email: 'express@alsea.com',
      company: companyA._id,
      saleType: 'retail',
    }).save();

    const branchHospitality = await new Branch({
      name: 'Café & Deli Fast-Casual',
      address: 'Plaza Central Local 5',
      tel: '555-2002',
      email: 'deli@alsea.com',
      company: companyA._id,
    ...
    const branchMixed = await new Branch({
      name: 'Mega Hipermercado',
      address: 'Av. Revolución 99',
      tel: '555-3003',
      email: 'mega@alsea.com',
      company: companyA._id,
      saleType: 'retail',
    ...
    const managerHosp = await new User({
      username: 'gerente_hosp',
      email: 'ghosp@alsea.com',
      password: hashedPassword,
      name: 'Gerente Hospitality',
      role: 'admin',
      companyId: companyA._id,
      branch: branchHospitality._id,
    }).save();
    await Branch.findByIdAndUpdate(branchHospitality._id, { manager: managerHosp._id });

    const cashierRetail = await new User({
      username: 'cajero_retail',
      email: 'cajero1@express.com',
      password: hashedPassword,
      name: 'Cajero Retail (Escáner)',
      role: 'user',
      companyId: companyA._id,
      branch: branchRetail._id,
    }).save();

    const cashierHosp1 = await new User({
      username: 'cajero_hosp',
      email: 'cajero1@deli.com',
      password: hashedPassword,
      name: 'Cajero Hospitality (Touch)',
      role: 'user',
      companyId: companyA._id,
      branch: branchHospitality._id,
    }).save();
    ...
    const cashierHosp2 = await new User({
      username: 'cajero_hosp2',
      email: 'cajero2@deli.com',
      password: hashedPassword,
    ...
    // Cajero Despedido
    await new User({
      username: 'cajero_malo',
      email: 'ladron@alsea.com',
      password: hashedPassword,
      name: 'Cajero Despedido',
      role: 'user',
      companyId: companyA._id,
      branch: branchRetail._id,
      active: false,
      deactivationReason: 'Faltante constante en caja fuerte.',
    }).save();
    ...
    const supplierPremium = await new Supplier({
      name: 'Alimentos Premium Corp',
      company: companyA._id,
      contactInfo: { email: 'premium@food.com', phone: '555-8888', address: 'Bodega 2' },
    }).save();
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const p = await new Product({
        ...
        isComposite: false,
        sku: `SKU-${1000 + i}`,
        supplier: supplierNacional._id,
        categories: [i % 2 === 0 ? catBebidas._id : catSnacks._id],
        company: companyA._id,
      }).save();
      ...
      await new InventoryItem({
        name: p.name,
        company: companyA._id,
        branch: branchMixed._id,
        product: p._id,
        stock: Math.floor(Math.random() * 500) + 50,
        costPrice: 9 + (i % 5),
        sellingPrice: 19 + (i % 10),
        measurement: 'unit',
        supplier: supplierNacional._id,
      }).save();
    }
    ...
    const rmCarne = await new RawMaterial({
      name: 'Carne Molida Premium',
      company: companyA._id,
      measurementUnit: 'g',
    }).save();
    const rmPan = await new RawMaterial({
      name: 'Pan de Hamburguesa',
      company: companyA._id,
      measurementUnit: 'unit',
    }).save();
    const rmQueso = await new RawMaterial({
      name: 'Queso Cheddar',
      company: companyA._id,
      measurementUnit: 'unit',
    }).save();
    const rmCafeG = await new RawMaterial({ name: 'Café Grano', company: companyA._id, measurementUnit: 'g' }).save();
    const rmLeche = await new RawMaterial({
      name: 'Leche Deslactosada',
      company: companyA._id,
      measurementUnit: 'ml',
    }).save();
    const rmTomate = await new RawMaterial({
      name: 'Tomate Bola',
      company: companyA._id,
      measurementUnit: 'unit',
    }).save();
    ...
    await new InventoryItem({
      ...
      company: companyA._id,
      branch: branchHospitality._id,
      rawMaterial: rmCarne._id,
      stock: 15000,
      costPrice: 0.1,
      measurement: 'g',
      supplier: supplierPremium._id,
    }).save();
    await new InventoryItem({
      name: rmPan.name,
      company: companyA._id,
      branch: branchHospitality._id,
      rawMaterial: rmPan._id,
      stock: 500,
      costPrice: 5,
      measurement: 'unit',
      supplier: supplierPremium._id,
      ...
      sizes: [
        {
          ...
          priceModifier: 0,
          ingredients: [
            { ingredient: rmCarne._id, quantity: 300 },
            { ingredient: rmPan._id, quantity: 1 },
            { ingredient: rmQueso._id, quantity: 2 },
            { ingredient: rmTomate._id, quantity: 0.5 },
          ],
        },
      ],
    }).save();
    ...
    const pBurger = await new Product({
      ...
      isComposite: true,
      sku: 'H-001',
      categories: [catComida._id],
      company: companyA._id,
      recipe: recetaBurger._id,
      price: 120,
    }).save();

    const pCafe = await new Product({
      name: 'Capuccino Caliente',
      brand: 'Deli House',
      isComposite: true,
      sku: 'C-001',
      categories: [catBebidas._id],
      company: companyA._id,
      recipe: recetaCafe._id,
      price: 45,
    }).save();
    ...
    await new StockTransfer({
      company: companyA._id,
      product: retailProducts[0]._id,
      fromBranch: branchMixed._id,
      toBranch: branchRetail._id,
      quantity: 50,
      status: 'completed',
      createdBy: ceoA._id,
      notes: 'Apoyo a sucursal express',
    }).save();

    await new StockTransfer({
      company: companyA._id,
      product: retailProducts[1]._id,
      fromBranch: branchRetail._id,
      toBranch: branchHospitality._id,
      quantity: 10,
      status: 'pending',
      createdBy: cashierRetail._id,
      notes: 'Falta agua embotellada en Deli',
    }).save();
    ...
    const physRegRetail = await new PhysicalRegister({
      name: 'Caja 1 Express',
      company: companyA._id,
      branch: branchRetail._id,
    }).save();
    const physRegHosp1 = await new PhysicalRegister({
      name: 'Touch POS 1 Deli',
      company: companyA._id,
      branch: branchHospitality._id,
    }).save();
    const physRegHosp2 = await new PhysicalRegister({
      name: 'Touch POS 2 Deli',
      company: companyA._id,
      branch: branchHospitality._id,
    }).save();

    const sessionOpenHosp1 = await new CashRegister({
      user: cashierHosp1._id,
      physicalRegister: physRegHosp1._id,
      company: companyA._id,
      branch: branchHospitality._id,
      initialAmount: 1000,
      expectedAmount: 1000,
      closed: false,
      startDate: new Date(Date.now() - 3600000), // Hace 1 hora
    ...
    const sessionOpenHosp2 = await new CashRegister({
      user: cashierHosp2._id,
      physicalRegister: physRegHosp2._id,
      company: companyA._id,
      branch: branchHospitality._id,
      initialAmount: 500,
      expectedAmount: 500,
      closed: false,
      startDate: new Date(Date.now() - 7200000), // Hace 2 horas
      payments: { cash: 0, credit: 0, debit: 0 },
      expenses: [],
      sales: [],
    }).save();
    ...
    const sessionClosedRetail = await new CashRegister({
      user: cashierRetail._id,
      physicalRegister: physRegRetail._id,
      company: companyA._id,
      branch: branchRetail._id,
      initialAmount: 2000,
      expectedAmount: 5500,
      ...
      payments: { cash: 2500, credit: 1000, debit: 0 },
      expenses: [],
      sales: [],
      notes: 'Caja entregada perfecta.',
    }).save();
    ...
    await new PendingOrder({
      user: cashierHosp1._id,
      cashRegister: sessionOpenHosp1._id,
      company: companyA._id,
      branch: branchHospitality._id,
      table: 'Mesa 4',
      ...
      productsSold: [
        {
          ...
          sizeName: 'Único',
          modifications: [{ name: 'Sin Cebolla', extraPrice: 0 }],
        },
        {
          product: pCafe._id,
          quantity: 1,
      ...
      total: 285,
      prepStartedAt: new Date(Date.now() - 600000),
    }).save();

    await new PendingOrder({
      user: cashierHosp2._id,
      cashRegister: sessionOpenHosp2._id,
      company: companyA._id,
      branch: branchHospitality._id,
      clientName: 'Auto Rojo',
      type: 'drive_thru',
      driveThruDetails: { lane: 1, carDescription: 'Sedan Rojo', licensePlate: 'XYZ' },
      kitchenStatus: 'ready',
      paymentStatus: 'paid',
      payments: [{ method: 'cash', amount: 120, date: new Date() }],
      productsSold: [
        {
          product: pBurger._id,
          quantity: 1,
      ...
      total: 120,
      prepStartedAt: new Date(Date.now() - 1200000),
      prepFinishedAt: new Date(Date.now() - 60000),
    }).save();
    ...
    for (let i = 0; i < 5; i++) {
      pastSessions.push(
        await new CashRegister({
          ...
          endDate: new Date(Date.now() - 86400000 * (i + 1) + 3600000 * 8),
          payments: { cash: 0, credit: 0, debit: 0 },
          expenses: [],
          sales: [],
        }).save(),
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const sale = await new Sale({
        ...
        cashRegister: targetSession._id,
        company: companyA._id,
        branch: targetBranch,
        total: subtotal,
        discount: 0,
        ...
        date: saleDate,
        customer: Math.random() > 0.8 ? customerSilver._id : undefined,
      }).save();
    ...
    console.log(
      `✅ SUPER SEED V3 FINALIZADO. Se insertaron ${totalGlobalSales} ventas, comandas vivas, y el entorno B2B manual completo.`,
    );
  } catch (error) {
