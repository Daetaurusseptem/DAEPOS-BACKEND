...
async function seed() {
  try {
    ...
    const companyAdmin = await new User({
      username: 'ceo_grupogr',
      email: 'ceo@grupogr.com',
      password: hashedPassword,
      name: 'Arturo (CEO)',
      role: 'companyAdmin',
      companyId: corpCompany._id,
    }).save();
    ...
    const branchRetail = await new Branch({
      ...
      tel: '555-2001',
      email: 'express@grupogr.com',
      company: corpCompany._id,
      saleType: 'retail',
    }).save();
    ...
    const managerRetail = await new User({
      username: 'gerente_retail',
      email: 'gretail@grupogr.com',
      password: hashedPassword,
      name: 'Gerente Retail',
      role: 'admin',
      companyId: corpCompany._id,
    ...
    const rmCafeG = await new RawMaterial({
      name: 'Café Grano',
      company: corpCompany._id,
      measurementUnit: 'g',
    }).save();
    ...
    const recetaBurger = await new Recipe({
      ...
      description: 'Hamburguesa con doble carne y pan',
      company: corpCompany._id,
      sizes: [
        {
          name: 'Único',
    ...
    const physRegHosp = await new PhysicalRegister({
      name: 'Caja 1 Deli',
      company: corpCompany._id,
      branch: branchHospitality._id,
    }).save();
    ...
    const cashSessionHosp = await new CashRegister({
      user: cashierHosp._id,
      physicalRegister: physRegHosp._id,
      company: corpCompany._id,
      branch: branchHospitality._id,
      initialAmount: 2000,
      expectedAmount: 2000,
    ...
    await new PendingOrder({
      user: cashierHosp._id,
      cashRegister: cashSessionHosp._id,
      company: corpCompany._id,
      branch: branchHospitality._id,
      table: 'Mesa 4',
      ...
      total: 280,
      prepStartedAt: new Date(Date.now() - 600000), // Hace 10 mins
    }).save();
    ...
    await new PendingOrder({
      ...
      company: corpCompany._id,
      branch: branchHospitality._id,
      clientName: 'Auto Rojo (Placa XYZ)',
      type: 'drive_thru',
      driveThruDetails: { lane: 1, carDescription: 'Sedan Rojo', licensePlate: 'XYZ-123' },
      kitchenStatus: 'ready',
      paymentStatus: 'paid', // Pagado en la primer ventanilla
    ...
    await new PendingOrder({
      ...
      company: corpCompany._id,
      branch: branchHospitality._id,
      clientName: 'Uber Eats - Orden #991',
      type: 'delivery',
      deliveryDetails: { platform: 'uber_eats', orderId: 'UBER-991', courierName: 'Repartidor Juan' },
      kitchenStatus: 'in_kitchen',
      paymentStatus: 'unpaid',
      payments: [],
      productsSold: [
        {
          ...
          status: 'sent_to_kitchen',
          sizeName: 'Único',
          modifications: [{ name: 'Extra Tocino', extraPrice: 10 }],
        },
      ],
      total: 390,
    ...
    await new PendingOrder({
      ...
      total: 135,
      prepStartedAt: new Date(Date.now() - 3600000),
      prepFinishedAt: new Date(Date.now() - 3000000),
    }).save();
    ...
    const saleRetail = await new Sale({
      user: cashierRetail._id,
      cashRegister: cashSessionRetail._id,
      company: corpCompany._id,
      branch: branchRetail._id,
      total: 56,
      discount: 0,
    ...
    const saleHosp = await new Sale({
      ...
      company: corpCompany._id,
      branch: branchHospitality._id,
      customer: customerGold._id,
      pointsEarned: 16, // 165 / 10 = 16 puntos
      total: 165,
      discount: 0,
