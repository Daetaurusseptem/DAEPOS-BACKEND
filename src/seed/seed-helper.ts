...
export async function runSeed(alreadyConnected: boolean = true): Promise<void> {
  try {
    ...
    const demoCompany = await new Company({
      name: 'Super Mercado Premium',
      description: 'Empresa Multi-sucursal Corporativa',
      address: 'Av. Corporativa 101',
      tel: '555-9000',
      email: 'owner@superpremium.com',
      adminId: sysadmin._id,
      maxActiveRegisters: 10,
      maxCashLimit: 5000,
    }).save();
    ...
    const branch2 = await new Branch({
      name: 'Sucursal Norte',
      address: 'Plaza Norte L4',
      tel: '555-0002',
      email: 'norte@superpremium.com',
      company: demoCompany._id,
    ...
    for (let i = 1; i <= 3; i++) {
      users.push(
        await new User({
          username: `cajero_norte${i}`,
          email: `cajero_norte${i}@norte.com`,
          password: hashedPassword,
          name: `Cajero Norte ${i}`,
          role: 'user',
          companyId: demoCompany._id,
    ...
    await new User({
      username: 'cocinero1',
      email: 'cocinero1@norte.com',
      password: hashedPassword,
      name: 'Chef Ejecutivo Carlos',
      role: 'kitchen',
      companyId: demoCompany._id,
      branch: branch2._id,
      isDemo: true,
    ...
    for (let i = 1; i <= 15; i++) {
      suppliers.push(
        await new Supplier({
          name: `Distribuidora Logística ${i}`,
          company: demoCompany._id,
    ...
    for (let i = 1; i <= 3; i++) {
      registers.push(
        await new PhysicalRegister({ name: `Caja ${i} Centro`, company: demoCompany._id, branch: branch1._id }).save(),
      );
    }
    for (let i = 1; i <= 2; i++) {
      registers.push(
        await new PhysicalRegister({ name: `Caja ${i} Norte`, company: demoCompany._id, branch: branch2._id }).save(),
      );
    }
    ...
    const openSession = await new CashRegister({
      user: users[0]._id,
      physicalRegister: registers[0]._id,
      company: demoCompany._id,
      branch: branch1._id,
      initialAmount: 2000,
      expectedAmount: 2000,
    ...
    const saturatedSession = await new CashRegister({
      user: users[1]._id,
      physicalRegister: registers[1]._id,
      company: demoCompany._id,
      branch: branch1._id,
      initialAmount: 2000,
      expectedAmount: 12500,
      closed: false,
      startDate: new Date(Date.now() - 3600000 * 4), // 4 horas abierta
      payments: { cash: 8500, credit: 1500, debit: 500 },
      expenses: [
        {
          amount: 500,
          reason: 'Pago de gas a proveedor externo',
          type: 'expense',
    ...
    const squaredSession = await new CashRegister({
      user: users[2]._id,
      physicalRegister: registers[2]._id,
      company: demoCompany._id,
      branch: branch1._id,
      initialAmount: 1500,
      expectedAmount: 4800,
      ...
      endDate: new Date(Date.now() - 86400000 * 2 + 3600000 * 8), // Hace 2 días, duró 8 horas
      payments: { cash: 2300, credit: 600, debit: 400 },
      expenses: [],
      sales: [],
      notes: 'Caja entregada en perfecto orden sin novedades.',
    }).save();
    ...
    const deficitSession = await new CashRegister({
      ...
      endDate: new Date(Date.now() - 86400000 * 3 + 3600000 * 7),
      payments: { cash: 3150, credit: 300, debit: 200 },
      expenses: [],
      sales: [],
      notes: 'Descuadre de $200 pesos al final del turno. Probable error al dar cambio.',
    }).save();
    ...
    const surplusSession = await new CashRegister({
      ...
      endDate: new Date(Date.now() - 86400000 * 1 + 3600000 * 6),
      payments: { cash: 1400, credit: 200, debit: 100 },
      expenses: [],
      sales: [],
      notes: 'Sobraron $120 pesos en propinas que un cliente dejó en el mesón.',
    }).save();
    ...
    const expensesSession = await new CashRegister({
      ...
      endDate: new Date(Date.now() - 86400000 * 4 + 3600000 * 9),
      payments: { cash: 3000, credit: 300, debit: 200 },
      expenses: [
        {
          amount: 1500,
          reason: 'Retiro parcial preventivo por supervisor',
          type: 'withdrawal',
          timestamp: new Date(Date.now() - 86400000 * 4 + 3600000 * 3),
        },
        {
          amount: 500,
          reason: 'Compra urgente de bolsas de hielo',
          type: 'expense',
          timestamp: new Date(Date.now() - 86400000 * 4 + 3600000 * 6),
        },
      ],
      sales: [],
      notes: 'Turno finalizado. Se ejecutaron retiros por seguridad del efectivo acumulado.',
    }).save();
    ...
    for (let i = 1; i <= 60; i++) {
      ...
      await new InventoryItem({
        ...
        costPrice: type.price * 0.6,
        sellingPrice: type.price + (i % 5),
        measurement: 'unit',
        supplier: suppliers[i % suppliers.length]._id,
    ...
    const lecheRM = await new RawMaterial({
      name: 'Leche Entera',
      description: 'Leche entera pasteurizada premium',
      company: demoCompany._id,
      measurementUnit: 'ml',
    }).save();
    ...
    const cafeRM = await new RawMaterial({
      name: 'Café en Grano',
      description: 'Café de especialidad en grano tostado',
      company: demoCompany._id,
      measurementUnit: 'g',
    }).save();
    ...
    await new InventoryItem({
      name: capuccinoProduct.name,
      company: demoCompany._id,
      branch: branch1._id,
      product: capuccinoProduct._id,
      stock: 0,
      costPrice: 5,
      sellingPrice: 45,
      measurement: 'unit',
