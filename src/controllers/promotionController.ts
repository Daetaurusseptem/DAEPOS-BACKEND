...
export const createPromotion = async (req: Request, res: Response) => {
    try {
        ...
        const { code, description, type, value, minPurchaseAmount, startDate, endDate, usageLimit, targetBranches, targetCategories } = req.body;
        ...
        if (existingPromo) {
            return res.status(400).json({ ok: false, message: 'Ya existe una promoción activa o inactiva con este código en la compañía' });
        }
...
export const validateDiscountCode = async (req: Request, res: Response) => {
    try {
        ...
        if (promotion.targetBranches && promotion.targetBranches.length > 0) {
            if (!branchId) {
                return res.status(400).json({ ok: false, message: 'Se requiere especificar la sucursal para validar este cupón' });
            }
...
export const updatePromotion = async (req: Request, res: Response) => {
    try {
        const { id } = req.params;
        const { description, type, value, minPurchaseAmount, startDate, endDate, usageLimit, isActive, targetBranches, targetCategories } = req.body;
