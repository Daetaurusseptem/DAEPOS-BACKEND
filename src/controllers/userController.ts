...
export const getAllNonAdminUsersOfCompany = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const users = await User.find(query).populate('branch', 'name').skip(skip).limit(limitNumber).exec();
...
export const deleteUser = async (req: Request, res: Response) => {
  ...
  if (adminCompany.length > 0) {
    return res
      .status(403)
      .json({ ok: false, msg: 'El elemento tiene referencias asignadas a el, eliminalas!', adminCompany });
  }
    ...
    if (active) {
      ...
      if (user.branch) {
        ...
        if (branch && branch.isActive === false) {
          return res.status(403).json({
            error:
              'No puedes reactivar a este usuario porque la sucursal a la que pertenece está desactivada. Edita su perfil y asígnalo a una sucursal activa primero.',
          });
        }
      }
      ...
      if (user.companyId) {
        ...
        if (
          ...
          const activeUsersCount = await User.countDocuments({
            ...
            active: true,
            role: { $ne: 'companyAdmin' },
          });
          if (activeUsersCount >= company.currentLimits.maxUsers) {
            return res.status(403).json({
              error: `Límite de usuarios (${company.currentLimits.maxUsers}) alcanzado en tu plan actual. Desactiva a otro empleado para liberar un espacio o mejora tu plan.`,
            });
          }
        }
      }
