...
export const createBranch = async (req: Request, res: Response) => {
    try {
        ...
        if (companyId) {
            ...
            if (company && company.currentLimits && company.currentLimits.maxBranches !== undefined && company.currentLimits.maxBranches !== -1) {
    ...
    try {
        ...
        if (req.body.isActive === true && branchToUpdate.isActive === false) {
            ...
            if (company && company.currentLimits && company.currentLimits.maxUsers !== undefined && company.currentLimits.maxUsers !== -1) {
                ...
                const usersToReactivate = await User.find({ branch: req.params.id, active: false, deactivationReason: 'Sucursal desactivada' });
                ...
                if (usersToReactivate.length > roomAvailable) {
                    ...
                    await User.updateMany({ branch: req.params.id, active: false, deactivationReason: 'Sucursal desactivada' }, { active: true, deactivationReason: '' });
                }
            } else {
                await User.updateMany({ branch: req.params.id, active: false, deactivationReason: 'Sucursal desactivada' }, { active: true, deactivationReason: '' });
            }
            ...
            await User.updateMany({ branch: req.params.id, active: true }, { active: false, deactivationReason: 'Sucursal desactivada' });
        }
