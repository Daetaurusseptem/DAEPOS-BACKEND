...
export const updateRestockStatus = async (req: Request, res: Response) => {
  try {
    ...
    // Lógica de Reabastecimiento Automático (Incrementar Inventario al completar o auditar)
    if ((status === 'completed' || status === 'pending_audit') && (oldStatus !== 'completed' && oldStatus !== 'pending_audit')) {
        ...
        for (const item of itemsToProcess) {
          ...
          if (inventoryItem) {
            ...
            const newInv = new InventoryItem({
              ...
              rawMaterial:
                itemTypeLower === 'rawmaterial' || itemTypeLower === 'raw_material' ? item.itemRef : undefined,
            });
            await newInv.save();
          }
        }
      ...
      if (payFromRegister) {
        try {
          ...
          if (activeRegister) {
            ...
            const supplierName = supplier ? supplier.name : 'Proveedor';
            const itemsToProcess = items || restock.items;
            const totalCost = (itemsToProcess || []).reduce(
              (sum: number, it: any) => sum + (it.quantity || 0) * (it.costPrice || 0),
              0,
            );
...
export const getCompanySupplierAgreements = async (req: Request, res: Response) => {
  try {
    ...
    const agreements = await SupplierAgreement.find(query).populate('supplier').populate('product').populate('branch');
...
export const resolveAgreedCostEndpoint = async (req: Request, res: Response) => {
  try {
    ...
    if (!companyId || !product || !supplier) {
      return res
        .status(400)
        .json({ ok: false, message: 'Faltan parámetros requeridos (companyId, product, supplier)' });
    }
    ...
    if (branch) {
      ...
      if (branchAgreement) {
        return res
          .status(200)
          .json({ ok: true, agreedCost: branchAgreement.agreedCost, level: 'branch', agreement: branchAgreement });
      }
    }
    ...
    if (generalAgreement) {
      return res
        .status(200)
        .json({ ok: true, agreedCost: generalAgreement.agreedCost, level: 'general', agreement: generalAgreement });
    }
...
export const getPendingAudits = async (req: Request, res: Response) => {
  try {
    const { companyId } = req.params;
    const audits = await SupplierRestock.find({ company: companyId, status: 'pending_audit' })
      .populate('items.itemRef');
...
export const resolveInventoryAudit = async (req: Request, res: Response) => {
  try {
    const { auditId } = req.params;
    const { items, supplierId, notes } = req.body; 
    // items: array de { type, itemRef, quantity (la que se había agregado), newCostPrice }

    const audit = await SupplierRestock.findById(auditId);
    if (!audit || audit.status !== 'pending_audit') {
      return res.status(404).json({ ok: false, message: 'Auditoría no encontrada o ya resuelta' });
    }

    if (supplierId) {
      audit.supplier = supplierId;
    }
    if (notes) {
      audit.notes = (audit.notes || '') + '\\nResolución: ' + notes;
    }

    // Actualizar el costo de los items en el inventario
    for (const auditedItem of items) {
      // Find matching item in audit.items to update its logged cost
      const auditItemRecord = audit.items.find(i => i.itemRef.toString() === auditedItem.itemRef.toString());
      if (auditItemRecord) {
        auditItemRecord.costPrice = auditedItem.newCostPrice;
      }

      // We need to re-adjust the weighted average cost in the InventoryItem.
      // Since it was added with $0 cost before, it diluted the average cost.
      // We will just do a simple overwrite or recalculate if possible. 
      // For simplicity, we just set the costPrice of the inventory to the new Cost Price.
      const itemTypeLower = auditedItem.type.toLowerCase();
      let inventoryItem;
      if (itemTypeLower === 'product') {
        inventoryItem = await InventoryItem.findOne({ product: auditedItem.itemRef, branch: audit.branch });
      } else {
        inventoryItem = await InventoryItem.findOne({ rawMaterial: auditedItem.itemRef, branch: audit.branch });
      }

      if (inventoryItem) {
        inventoryItem.costPrice = auditedItem.newCostPrice;
        await inventoryItem.save();
      }
    }
    ...
    return res.status(200).json({ ok: true, audit, message: 'Ajuste auditado resuelto' });
  } catch (error) {
    return res.status(500).json({ ok: false, message: error });
  }
};
